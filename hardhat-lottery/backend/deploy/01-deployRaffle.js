...
module.exports = async ({ getNamedAccounts, deployments }) => {
    ...
    if (developmentChains.includes(network.name)) {
        ...
        // Fund the contract with 
        await vrfCoordinatorV2.fundSubscription(subscriptionId.toNumber(), VRF_SUB_FUND_AMOUNT);
    ...
    if (developmentChains.includes(network.name)) {
        const contractAddress = (await deployments.get('VRFCoordinatorV2Mock')).address;
        const vrfCoordinatorV2 = await ethers.getContractAt('VRFCoordinatorV2Mock',contractAddress);
