...
const Header = () => {
    ...
    return (
        <>
            <nav className={`${scrolling ? 'bg-white text-blue-500' : 'text-white bg-gradient-to-r from-gray-600 to-gray-800'} py-4 px-6 flex items-center justify-between shadow-lg fixed top-0 left-0 right-0`}>
                <Link to='/' className="flex items-center" role='button'>
                    <svg width="30" height="30" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
                        <circle cx="50" cy="50" r="45" fill="#FF6C02" />
                        <rect x="40" y="25" width="20" height="50" fill="#FFFFFF" />
                        <rect x="35" y="40" width="30" height="20" fill="#FFFFFF" />
                        <text x="50" y="70" text-anchor="middle" font-family="Arial" font-size="12" fill="white">NFT</text>
                    </svg>


                    <span className='ml-3 md:block lg:block hidden font-extrabold text-xl bg-none hover:bg-none'>NFT Marketplace</span>
                </Link>
                {(!isWalletConnected) ? <button onClick={() => setConnectModal(true)} className="flex items-center bg-gradient-to-r from-light to-gray-100 text-white font-semibold py-2 px-4 rounded-lg shadow-md">
